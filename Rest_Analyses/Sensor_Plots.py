...
def calc_psds(script_dir, 
    ...
    for sub in sub_folders:
        ...
        if not ses_path.exists():
            return f"Session path does not exist for {sub}/{ses}, skipping."
        
        rest_dir = ses_path / 'rest'
        if not rest_dir.exists():
            return f"Rest directory does not exist for {sub}/{ses}, skipping."
        ...
        if not epo_files:
            return f"No epoch files found for {sub}/{ses}, skipping."
...
if __name__ == "__main__":
    ...
    calc_psds(
        script_path, 
        sub_folders=['sub-67'],  # You can specify subjects or leave None for auto-detect
        sessions=['ses-1'],
        sensor_types=['mag', 'grad'],
        n_processes=1  # Adjust based on your system
    )
