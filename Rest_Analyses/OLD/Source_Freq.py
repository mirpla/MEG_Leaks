#%% Initial draft for source frequency analysis in MEG resting-state data
# very inefficient, use Source_PSD_HPC.py instead for all subject calculations
import mne
import numpy as np
import h5py
import pandas as pd
...
class MEGSourceAnalyzer:
    ...
    def __init__(self, base_path):
        ...
        self.cache_dir = self.data_path / 'derivatives' / 'spectral_analysis' / 'freq_cache'
...
def run_analysis_example():
    ...
    data = pd.read_csv(base_path/ 'Data' / 'Subject_Information.csv', encoding='latin1') 
    Included = ~data['Excluded'].astype(bool)
 
    subjects = data['sub'][Included].tolist()  # Update with your subject IDs
