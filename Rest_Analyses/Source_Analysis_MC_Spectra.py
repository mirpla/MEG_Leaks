...
'Scripts related to plotting the power-spectra across rest periods'
...
def plot_source_spectra(ss_flag, ms_flag, msul_flag, 
                        window_size = 3, 
        ...
        for side_idx, side in enumerate(LR): # left only for now
            ...
            for ct_idx, ct in enumerate(comp_t):   
                if ms_flag == 1:    
                    ...
                    fig2.suptitle(f'{ConIn} {side} Motor {ct} activity')
            
                if msul_flag == 1:                
                    fig3 = plot_multi_subject_components_unlocked(
                    data, sub_data, sub_lst, ct, col)
                    fig3.suptitle(f'{ConIn} {side} Motor {ct} activity')
