...
Pin31_WL_MEPs_6 = fliplr(logical(dec2bin  (31,8)-48)); % Use pin #31 to initialise the collection of MEPs at recall

Pin30_WL_TS_6 = fliplr(logical(dec2bin  (30,8)-48)); % % % Pin number #30 >> Main Coil at recall

Pin27_WL_CS_6 = fliplr(logical(dec2bin  (27,8)-48)); % % % Pin number #27 >> Conditioning Coil at recall
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   % if seq == 'wordlist-1.txt' % MAIN WORD LIST  
       
        for Block = 6 %%%%%Main block loop%%%%% %
...
Screen('Flip', mainwindow);
     ...
     for Trial = 1:20 %%%%%TMS stimuli%%%%%
            ...
            pp(uint8(2:9),Pin31_WL_MEPs_6,false,port,base_addr) %Write trigger 8
            WaitSecs (0.001);
...
Screen('Flip', mainwindow);
     ...
     for Trial = 1:12 %%%  Conditioning Stimulus Trigger  %%%
            pp(uint8(2:9),resettrig,false,port,base_addr); %close
            pp(uint8(2:9),Pin27_WL_CS_6,false,port,base_addr) %Write trigger 8
            WaitSecs (0.001);
            pp(uint8(2:9),resettrig,false,port,base_addr);
            ...
            pp(uint8(2:9),resettrig,false,port,base_addr); %close
            pp(uint8(2:9),Pin30_WL_TS_6,false,port,base_addr) %Write trigger 8
            WaitSecs (0.001);
            pp(uint8(2:9),resettrig,false,port,base_addr);
