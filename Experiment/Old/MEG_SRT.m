function MEG_SRT(SubInfo, path, debug, startBlock) 
...
savepath.restname       = [savepath.path, 'MEG_SRT_', 'p', num2str(SubInfo.subjID), '_Rest.txt'];
...
if debug == 1
    [window, ~] = PsychImaging('OpenWindow', screenNumber, windowcolor, [0 0 640 480]);
else
    [window, ~] = PsychImaging('OpenWindow', screenNumber, windowcolor);
end
...
Key{1,1} = KbName('7&'); % Right Index Finger 512
Key{1,2} = KbName('8*'); % Right Middle Finger 256
Key{1,3} = KbName('9('); % Right Ring Finger 128
Key{1,4} = KbName('0)'); % Right Pinky 64
Key{1,5} = KbName('5%'); % Left Thumb 2048
...
blocktrig = 1; % Indicating task is being performed coded as multipes of 256 on STI101

resttrig  = 2; % indicates official rest period coded as as multipes of 512 + x*512on STI101
ridx      = 2; % counter keeping track of which rest period is measured 
...
Screen('DrawTexture',window, tr_tx{1,6}, [], trigPos); % signal start of trial
Screen('Flip', window);
...
for block = startBlock:blocknum
    ...
    if restFlag == 1  || block == blocknum
        ...
        if MEG == 1
            ppdev_mex('Write',1,resttrig+ridx)
        end
